...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        if self.startingPosition in self.corners:
            return (self.startingPosition,tuple(x for x in self.corners if x!=self.startingPosition))
        else:
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            children += [( self.getNextState(state,action),action,self.getActionCost(state,action,self.getNextState(state,action)) )]
    ...
    def getNextState(self, state, action):
        ...
        if (nextx,nexty) in state[1]:
            return ((nextx,nexty),tuple(x for x in state[1] if x!=(nextx,nexty)))
        else:
...
def cornersHeuristic(state, problem):
    ...
    unvisited_corners=[corner for corner in state[1]] #Corners not visited in state
    position=state[0] # Current state position
    ...
    while len(unvisited_corners)!=0:
        ...
        closest_distance,closest_corner=min(distances, key = lambda y: y[0])
...
def foodHeuristic(state, problem):
    ...
    distances_saved=problem.heuristicInfo
    uneaten_pellets=foodGrid.asList() #Pellets not eaten in state
    game_state=problem.startingGameState

    if len(uneaten_pellets) == 0:
        return 0
    
    distances=[]
    for pellet in uneaten_pellets:
        if (position,pellet) in distances_saved:
            distance=distances_saved[(position,pellet)]
        else:
            distance=mazeDistance(position,pellet,game_state)
            distances_saved[(position,pellet)]=distance
